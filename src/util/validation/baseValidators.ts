...
const escapeHtmlString = (value: string, helpers: Joi.CustomHelpers): string => {
  if (typeof value !== 'string') helpers.error('string.base')
  return sanitizeHtml(value)
}
export const stringLenghtValidation = (schema: Joi.StringSchema, min?: number, max?: number): StringSchema => {
  const minSchema = (typeof min === 'number') ? schema.min(min) : schema
  return (typeof max === 'number') ? minSchema.max(max) : minSchema
}

export const dateLenghtValidation = (schema: Joi.DateSchema, min?: string | Date, max?: string | Date): DateSchema => {
  const lessSchema = (typeof min === 'number') ? schema.less(min) : schema
  return (typeof max === 'number') ? lessSchema.greater(max) : lessSchema
}

export const stringValidation = ({ name, min, max }: IStringValidationOptions): StringSchema => {
...
export const dateValidation = ({ name, min, max }: IDateValidationOptions): DateSchema => {
